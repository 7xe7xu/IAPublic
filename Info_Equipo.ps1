...
function Format-Content {
    ...
    Write-Host "Contenido recibido en Format-Content:"
    Write-Host $content
    Write-Host "--------------------"
...
"@
    ...
    $infoSecciones = @(
        @{Titulo="Resumen del sistema"; Contenido=(Consultar-ResumenSistema | ConvertTo-Html -Fragment)}
        @{Titulo="Placa base"; Contenido=(Consultar-InfoPlacaBase | ConvertTo-Html -Fragment)}
        @{Titulo="CPU"; Contenido=(Consultar-InfoCPU | ConvertTo-Html -Fragment)}
        @{Titulo="Memoria RAM"; Contenido=(Consultar-InfoMemoria | ConvertTo-Html -Fragment)}
        @{Titulo="Discos"; Contenido=(Consultar-InfoDiscos | ConvertTo-Html -Fragment)}
        @{Titulo="Adaptadores de red"; Contenido=(Consultar-InfoRed | ConvertTo-Html -Fragment)}
        @{Titulo="GPU"; Contenido=(Consultar-InfoGPU | ConvertTo-Html -Fragment)}
        @{Titulo="Dispositivos USB registrados en el sistema"; Contenido=(Consultar-InfoUSB | ConvertTo-Html -Fragment)}
        @{Titulo="Baterías"; Contenido=(Consultar-InfoBateria | ConvertTo-Html -Fragment)}
        @{Titulo="Actualizaciones"; Contenido=(Consultar-Actualizaciones | ConvertTo-Html -Fragment)}
        @{Titulo="Sistema operativo"; Contenido=(Consultar-InfoSO | ConvertTo-Html -Fragment)}
        @{Titulo="Usuarios"; Contenido=(Consultar-InfoUsuarios | ConvertTo-Html -Fragment)}
        @{Titulo="Políticas y grupos"; Contenido=(Consultar-Políticas | ConvertTo-Html -Fragment)}
        @{Titulo="Arranque"; Contenido=(Consultar-Arranque | ConvertTo-Html -Fragment)}
        @{Titulo="Energía"; Contenido=(Consultar-Energia | ConvertTo-Html -Fragment)}
        @{Titulo="Seguridad"; Contenido=(Consultar-Seguridad | ConvertTo-Html -Fragment)}
        @{Titulo="Software instalado ordenado por fabricante"; Contenido=(Consultar-InfoSoftware -PorFabricante | ConvertTo-Html -Fragment)}
    )

    # Depuración: Imprimir el contenido de cada sección
    foreach ($seccion in $infoSecciones) {
        Write-Host "Sección: $($seccion.Titulo)"
        Write-Host "Contenido: $($seccion.Contenido)"
        Write-Host "--------------------"
    }
    ...
    $rutaExportacion = Dibujar-Exportacion
    if ($rutaExportacion) {
        $infoSecciones = Recopilar-InformacionSistema
        $htmlContent = Get-HTMLContent
        $htmlContent | Out-File -FilePath $rutaExportacion -Encoding UTF8
        Write-Host "Informe exportado exitosamente a: $rutaExportacion" -ForegroundColor Green
    } else {
        Write-Host "Exportación cancelada." -ForegroundColor Yellow
    }
}
